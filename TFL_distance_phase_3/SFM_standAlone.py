...
from TFL_distance_phase_3 import SFM
...
def visualize(prev_container, curr_container, focal, pp):
    ...
    prev_sec.set_title('prev(' + str(prev_container.prev_frame_id) + ')')
    prev_sec.imshow(prev_container.img)
    prev_p = prev_container.traffic_light
    prev_sec.plot(prev_p[:, 0], prev_p[:, 1], 'b+')

    curr_sec.set_title('curr(' + str(prev_container.curr_frame_id) + ')')
    ...
    for i in range(len(curr_p)):
        ...
        if curr_container.valid[i]:
            curr_sec.text(curr_p[i, 0], curr_p[i, 1], r'{0:.1f}'.format(
                curr_container.traffic_lights_3d_location[i, 2]), color='r')
...
# prev_img_path = 'data/frames/dusseldorf_000049_0000' + str(prev_frame_id) + '_leftImg8bit.png'
# curr_img_path = 'data/frames/dusseldorf_000049_0000' + str(curr_frame_id) + '_leftImg8bit.png'
