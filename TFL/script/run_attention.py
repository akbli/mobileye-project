...
def get_kernel():
    return np.array([[-2 / 324, -2 / 324, -2 / 324, -2 / 324, -2 / 324, -2 / 324, -2 / 324, -2 / 324, -2 / 324, -2 / 324, -2 / 324, -2 / 324, -2 / 324, -2 / 324, -2 / 324, -2/324, -2/324, -2/324],
...
def convolution(image, color_image, RGB):
    kernel = get_kernel()
    image_after_conv = sg.convolve2d(color_image, kernel, boundary='symm', mode='same')
    # x_red, y_red = find_one_color_lights(red_image_after_conv, image)
    max_lights_image = ndimage.maximum_filter(image_after_conv, size=5)
    plt.imshow(max_lights_image)
    fig, (max_mag) = plt.subplots(1, 1, figsize=(6, 15))
    max_mag.imshow(np.absolute(max_lights_image))
...
def find_tfl_lights(image, **kwargs):
    red_image = image[:, :, 0]
    green_image = image[:, :, 1]
    x_red, y_red = convolution(image, red_image, [255, 0, 0])
    x_green, y_green = convolution(image, green_image, [0, 255, 0])
